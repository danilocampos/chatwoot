...
RSpec.describe Internal::CheckNewVersionsJob do
  ...
  describe '#perform' do
    context 'when GitHub API returns successful response' do
      before do
        stub_request(:get, 'https://api.github.com/repos/danilocampos/chatwoot/releases/latest')
          .to_return(
            status: 200,
    ...
    context 'when not in production environment' do
      ...
      it 'does not make any API calls or update redis' do
        expect { job }.not_to change { Redis::Alfred.get(Redis::Alfred::LATEST_CHATWOOT_VERSION) }.from(nil)
        expect(a_request(:get, 'https://api.github.com/repos/danilocampos/chatwoot/releases/latest')).not_to have_been_made
      end
    end
